...
import { signJWT, verifyJWT } from "../middleware/signJWT";
...
const accessTokenCookiesOptions: CookieOptions = {
  ...
  expires: new Date(
    Date.now() + config.auth.accessTokenExpiresInMinutes * 60 * 1000
...
export const loginHandler = async (
  ...
  try {
    ...
    if (cookies) {
      RefreshTokenArr = RefreshTokenArr?.filter((RT: string) => RT !== cookies);
